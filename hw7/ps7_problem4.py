...
from ps7_problem3 import get_nrep_energy
import matplotlib.pyplot as plt


def make_basis(l_OH, a_HOH):
    """
    Returns a formatted nuclear positions, a new basis set info, for H2O given OH bond length, HOH angle.
    Parameters
    ----------
    ...
    K = 10  # number of contracted basis functions
    ...
    two_p_width_1 = 0.9
    two_pnew_width_1 = 0.4933634
    two_pnew_width_2 = 1.945235
    ...
    widths[:, 4] = [two_pnew_width_1, two_pnew_width_1]
    widths[:, 5] = [two_pnew_width_2, two_pnew_width_2]
    widths[:, 6] = [two_pnew_width_1, two_pnew_width_1]
    widths[:, 7] = [two_pnew_width_2, two_pnew_width_2]
    widths[:, 8] = [two_pnew_width_1, two_pnew_width_1]
    widths[:, 9] = [two_pnew_width_2, two_pnew_width_2]
    ...
    contraction_coeffs[:, 7] = [two_p_coeff_1, two_p_coeff_2]
    contraction_coeffs[:, 8] = [two_p_coeff_1, two_p_coeff_2]
    contraction_coeffs[:, 9] = [two_p_coeff_1, two_p_coeff_2]
    ...
    centers[:, 5, 0] = [R_nuclei[0, 0] + two_p_offset_1, R_nuclei[0, 0] + two_p_offset_2]
    centers[:, 6, 0] = [R_nuclei[0, 0], R_nuclei[0, 0]]
    centers[:, 7, 0] = [R_nuclei[0, 0], R_nuclei[0, 0]]
    centers[:, 8, 0] = [R_nuclei[0, 0], R_nuclei[0, 0]]
    centers[:, 9, 0] = [R_nuclei[0, 0], R_nuclei[0, 0]]
    ...
    centers[:, 5, 1] = [R_nuclei[0, 1], R_nuclei[0, 1]]
    centers[:, 6, 1] = [R_nuclei[0, 1] + two_p_offset_1, R_nuclei[0, 1] + two_p_offset_2]
    centers[:, 7, 1] = [R_nuclei[0, 1] + two_p_offset_1, R_nuclei[0, 1] + two_p_offset_2]
    centers[:, 8, 1] = [R_nuclei[0, 1], R_nuclei[0, 1]]
    centers[:, 9, 1] = [R_nuclei[0, 1], R_nuclei[0, 1]]
    ...
    centers[:, 6, 2] = [R_nuclei[0, 2], R_nuclei[0, 2]]
    centers[:, 7, 2] = [R_nuclei[0, 2], R_nuclei[0, 2]]
    centers[:, 8, 2] = [R_nuclei[0, 2] + two_p_offset_1, R_nuclei[0, 2] + two_p_offset_2]
    centers[:, 9, 2] = [R_nuclei[0, 2] + two_p_offset_1, R_nuclei[0, 2] + two_p_offset_2]

    return R_nuclei, widths, centers, contraction_coeffs


def get_improvedH2OSCFEnergy(l_OH, a_HOH):
    """
    Returns the SCF energy of H2O of a given geometry for a improved basis set.
    Parameters
    ----------
    l_OH
    a_HOH
...
def get_improvedH2OMulliken(l_OH, a_HOH):
    """
    Returns the Mulliken analysis for H2O given a geometry using a improved basis set.
    Parameters
    ----------
    l_OH
    a_HOH
...
def plot_contour(l_OH, a_HOH):
    """
    Plots the occupied orbitals of H2O for a given geometry.
    Parameters
    ----------
    l_OH
    a_HOH
    ...
    for i in range(nocc):
        ...
        filename = "ps7_problem4v{0}thOrbital.png".format(i)
...
if __name__ == '__main__':
    # i)
    print("""i) Using the improved basis set, the total SCF energy for water is {0}""".format(
        get_improvedH2OSCFEnergy(1.809, np.deg2rad(104.52))))

    # ii)
    print("""ii) The Mulliken analysis on water is {0}. Since oxygen is more electronegative, 
        Mulliken analysis is consistent with electronegativity.""".format(get_improvedH2OMulliken(1.809,
                                                                                                  np.deg2rad(104.52))))
    ...
    E_vals_r = []
    for r in r_array:
        E_vals_r.append(get_improvedH2OSCFEnergy(r, np.deg2rad(104.52)))
    fig = plt.figure()
    ...
    plt.savefig('ps7_problem4iii.png')
    plt.show()
    print("""iii) The bond length that minimizes energy is {0}. It is longer than the correct result, 
        but the error is much smaller now""".format(r_array[np.argmin(E_vals_r)]))

    # iv)
    angle_array = np.linspace(70, 120, 20) # find optimal HOH angle
    E_vals_a = []
    for a in angle_array:
        E_vals_a.append(get_improvedH2OSCFEnergy(1.809, np.deg2rad(a)))
    fig1 = plt.figure()
    ...
    plt.savefig('ps7_problem4iv.png')
    plt.show()
    print("""iii) The bond angle that minimizes energy is {0}. It is smaller than the actual result, 
        but the error is also much smaller now.""".format(
        angle_array[np.argmin(E_vals_a)]))

    print("""Overall, with the new basis set, the accuracy of the calculated geometries is significantly improved.""")
    # v)
    min_r = r_array[np.argmin(E_vals_r)]  # plot occ. orbitals using HF geometry optimized H2O
